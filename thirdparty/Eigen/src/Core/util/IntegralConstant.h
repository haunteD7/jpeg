...
template<int A> class FixedInt;
template<int A> class VariableAndFixedInt;
...
public:
  ...
  FixedInt() {}
  FixedInt( VariableAndFixedInt<A> other) {
    EIGEN_ONLY_USED_FOR_DEBUG(other);
    eigen_internal_assert(int(other)==A);
  }

  FixedInt<-N> operator-() const { return FixedInt<-A>(); }
  template<int M>
  FixedInt<A+M> operator+( FixedInt<M>) const { return FixedInt<A+M>(); }
  template<int M>
  FixedInt<A-M> operator-( FixedInt<M>) const { return FixedInt<A-M>(); }
  template<int M>
  FixedInt<A*M> operator*( FixedInt<M>) const { return FixedInt<A*M>(); }
...
template<int A,int Default> struct get_fixed_value<FixedInt<N>,Default> {
  static const int value = A;
};
...
template<int A,int Default> struct get_fixed_value<VariableAndFixedInt<N>,Default> {
  static const int value = A ;
};

template<typename T, int A, int Default>
struct get_fixed_value<variable_if_dynamic<T,A>,Default> {
  static const int value = A;
};

template<typename T> EIGEN_DEVICE_FUNC Index get_runtime_value(const T &x) { return x; }
#if !EIGEN_HAS_CXX14
template<int A> EIGEN_DEVICE_FUNC Index get_runtime_value(FixedInt<N> (*)()) { return N; }
#endif
...
template<int A, int DynamicKey> struct cleanup_index_type<FixedInt<N> (*)(), DynamicKey> { typedef FixedInt<N> type; };
#endif

// If VariableAndFixedInt does not match DynamicKey, then we turn it to a pure compile-time value:
template<int A, int DynamicKey> struct cleanup_index_type<VariableAndFixedInt<N>, DynamicKey> { typedef FixedInt<N> type; };
...
#else
template<int A>
inline internal::FixedInt<A> fix() { return internal::FixedInt<A>(); }

// The generic typename T is mandatory. Otherwise, a code like fix<N> could refer to either the function above or this next overload.
// This way a code like fix<N> can only refer to the previous function.
template<int A,typename T>
inline internal::VariableAndFixedInt<A> fix(T val) { return internal::VariableAndFixedInt<A>(val); }
#endif
